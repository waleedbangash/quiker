...
class EProviderController extends Controller
{
    ...
    public function store(CreateEProviderRequest $request)
    {
        ...
        try {
            ...
            $document_Name = time() . mt_rand(1000, 999999) . '_' .$document->getClientOriginalName();
            $document->move(public_path('document'),$document_Name);
    ...
    public function update(int $id, UpdateEProviderRequest $request)
    {
        ...
        try {
            ...
            $provider=$this->eProviderRepository->where('id',$id)->first();
            $document=$request->file("document");
            if(!empty($document))
            {
            $document_Name = time() . mt_rand(1000, 999999) . '_' .$document->getClientOriginalName();
